...
// These are strings we can pass in to our askQuestion function.
...
const answer = askQuestion(validQuestion);
console.log(answer)
...
const justAPromise = askQuestion(validQuestion)
...
const isThisAPerson = getPersonInQueue(2)
console.log(isThisAPerson); //> Promise { <pending> }

// However, we can use the `then` method to get the resolved value.

getPersonInQueue(1).then((personFromQueue) => {
  console.log('The person is: ' + personFromQueue)
}).catch((error) => {
  console.log(error)
})
...
getPersonInQueue("Colin").then((personFromQueue) => {
  console.log('The person is: ' + personFromQueue)
}).catch((error) => {
  console.log(error)
})