...
for (let i=0; i<dinosaurs.length; i++){
  const deadDino = dinosaurs[i] + ' is sadly extinct.';
  extinctDinos.push(deadDino);
}

const extinctDinos2 = dinosaurs.map(function(dino){
  return dino + ' is sadly extinct';
});
console.log(extinctDinos)
...
const numberedPies = dinosaurs.map(function (dino, i) {
  return `${i + 1}. ${dino} pie`
})
// array.filter()
...
const isFiveLetters = function(fruit){
  return fruit.length === 5;
}

const fiveLetterFruits2 = fruitsArr.filter(isFiveLetters);
// 22. Log the result:
...
const fiveLetterJellies = fruitsArr.filter(function (fruit) {
  return fruit.length === 5;
}).map(function (fruit) {
  return fruit +' jelly';
});
...
const smoothies = fruitsArr.map(function (fruit, i, fruits) {
  if (i === fruits.length - 1){
  return fruit + '-' + fruits[0];
  } else {
    return fruit + '-' + fruits[i+1];
  }
});
// 30. Console log the result:

// ['apple-banana', 'banana-cherry', 'cherry-grape', 'grape-lemon', 'lemon-lime', 'lime-mango', 'mango-papaya', 'papaya-peach', 'peach-orange', 'orange-apple']
...
const priceyMeats = entrees.filter(function (entree) {
  return !entree.vegetarian && entree.price >= 15;
});
...
const leanMeats = entrees.filter(function(entree1) {
  return !entree1.vegetarian && entree1.cals < 1000;
});
// END: Lesson 07.02
// NEXT: Lesson 07.03
