...
console.log('cafeTotal:',cafeTotal); // 5010
...
console.log('cafeTotal:', cafeTotal); // cafeTotal: 77
...
// + shortcut
// + right in front of a stringy number works like Number(), parseInt(), parseFloat()
...
let user = null;
console.log('user'), user, typeof(null);
...
kind = "Jack";
suit = "Hearts";
let imgPath = baseUrl + kind + "-of-" + suit + ".jpg";
...
let myPet = "Bunny", treat = "blueberries", meal = "breakfast";
...
dayTripPrice = '$120';
dayTripPrice = dayTripPrice.replace("$",""); //120
...
deliveryFee = 40;
// subTot = ;
let totalCost = ((pricePerKg * kilosRice) * percentTax) + deliveryFee;
...
let foodTot = 30;
bevTot = 20;
subTotal = foodTot + bevTot;
taxPct = 0.08; 
tipPct = 0.2;
taxTot = subTotal * taxPct;
tipTot = tipPct * subTotal;
grandTotal = subTotal + taxTot + tipTot;

// number.toFixed(2) rounds number to the nearest 2 decimal places. Also makes it a string, MUST be used after all math.
let guestCheck = `

    GUEST CHECK at ${placeName}:
        Food tot: $${foodTot.toFixed(2)}
        Bev tot: $${bevTot.toFixed(2)}
        Sub-tot: $${subTotal.toFixed(2)}
        Tax pct: ${taxPct*100}%
