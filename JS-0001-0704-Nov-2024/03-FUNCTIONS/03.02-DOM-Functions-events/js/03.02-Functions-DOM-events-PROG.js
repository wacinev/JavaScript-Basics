...
console.log('ck-box.checked;', ckBox.checked)
...
function greetUser() {
    ...
    //outBox.textContent = `Hello, ${firstName} ${lastName}!`;
    ...
    // B.
    // If only first or last name is provoided, just greet by that one name.
    // If NO name is provided -- if user clicked greet user button with out enterin any name -- greet as "good morning buddy" 
...
function pickFruit(event) {
    ...
    let choosenFruit = this.value;

    // 21. Output the chosen fruit in UPPERCASE:
    outBox.innerHTML = `You chose ${choosenFruit.toUpperCase()}! Enjoy!`;

    // 22. If the Add Snack checkbox is checked, add data-food value to output
    if(ckBox.checked) {
        ...
        //outBox.innerHTML += " at index " + indx; // You picked CHERRY at index 6
...
function calculate() {
    ...
    switch(op) {
        ...
        case "+":
            // 35. If this is the case, run code:
            ans = num1 + num2;
    ...
    //38B. calculate the answer using eval() instead of swtich case
    // let answer = eval(`${num1} ${op} ${num2}`);
    // outBox.textContent = answer;

    //38C. Make it all one line of code
