...
// 8. Call the login function on Log In button click
// loginBtn.addEventListener('click', logIn);

// save username and pswd as session variables on login btn
// mouseover -- that instant before submit btn is clicked
// also save session vars if submit button is reached via tabbing
// which would be a focus event
// and also save session vars if user just hits enter after typing
// pswd or username
...
pswdBox.addEventListener('change', function() {
    sessionStorage.setItem("user", userBox.value);
    sessionStorage.setItem("pswd", pswdBox.value);
    console.log('hello from pswd box submit (change)');
});

userBox.addEventListener('change', function() {
    sessionStorage.setItem("user", userBox.value);
    sessionStorage.setItem("pswd", pswdBox.value);
    console.log('hello from user box submit (change)');
});
...
function logIn(event) {
    ...
    // prevent btn click from refreshing pg and resetting input boxes
    // and if action page is specified, to keep you from redirecting there
    // event.preventDefault();
    ...
    // and password. There are two conditions, so use the && operator. 
    // If credentials match, output "Welcome" and hide input elements 
    if(user == USER && pswd == PSWD) {
        outputH2.textContent = `Welcome, ${user}!`; // output Welcome
