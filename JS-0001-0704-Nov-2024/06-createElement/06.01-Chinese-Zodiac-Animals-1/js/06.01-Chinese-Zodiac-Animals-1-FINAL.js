...
// 13. Reload the page. In the top middle of the section there should be white box that says 'Chinese Zodiac Animal'
...
// Start "Animal Div Maker Loop" :
for(let ani of animals) {

    // make a div to hold current animal
    const divvy = document.createElement('div');
    divvy.className = 'divvy'; // assign class
    section.appendChild(divvy); // output to section

    const pic = new Image(); // make image object
    // set its source to the current animal jpg
    pic.src = `images/animals/${ani.eng}.jpg`; 
    pic.className = 'animal-pic'; // assign class
    divvy.appendChild(pic); // output pic to div

    // make an input element to go under the animal pic:
    const inputBox = document.createElement('input');
    inputBox.placeholder = '?';

    // have input box call inline anon callback func on change (Enter or Tab):
    inputBox.addEventListener('change', function() {
        let word = inputBox.value;
        if(word == ani.eng || word == ani.also || word == ani.chi) {
            this.style.backgroundColor = '#282'; // set input box bg color green
        } else {
            this.style.backgroundColor = '#A22'; // set input box bg color red
        }
        this.style.color = '#fff'; // make input box text white to contrast w bg
    });
