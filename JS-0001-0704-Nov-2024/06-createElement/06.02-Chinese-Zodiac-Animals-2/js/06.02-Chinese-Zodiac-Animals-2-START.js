...
for(let i = 0; i < animals.length; i++) {
    ...
    // page in the browser. If the function referred to in the listener 
    // does not exist at that point, we'll get an error.

    // this keyword

    // Inside a function, the **this** keyword refers to the object that 
    // called the function. In the **checkSpelling** function, **this** 
    // is the input box:
    ...
    // 26. Define listener for sound icon which runs a fucntion 
    //that plays an mp3 on click
    ...
    // 34. Make a p-tag for holding the years as a vertical strip, 
    // one year per line
    ...
    // 37. Declare a start year variable and a string to hold all 
    // the years concatenated with spaces: "1880 1892 1904 1916 1928..."


    // 38. Run a for loop that starts 156 years before the current 
    // animal yaer and continues until the counter reaches the current 
    // animal year plus 12; counter goes up by 12 each time: 


    // 39. Output the yearsStr to the pTagYrs:


    // 40. Make hidden elements that shows English spelling 
    // but only when spelling is correct


    // 41. Make hidden elements that shows Pinyin spelling 
    //. but only when spelling is correct
