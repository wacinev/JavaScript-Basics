...
while (j < fruits.length) {
    jBeans.push(`${fruits[j]} jellybeans`);
    j++
}
...
//CHALLENGE: 5 unique numbers 1-69 (in ascending order) 1 number 1-26
// bonus: add leading "0" for single digit number and store into array

let lotteryTicket = [];

while (lotteryTicket.length<5) {
    let num = Math.ceil(Math.random()*69);
    while (lotteryTicket.includes(num)){
        num = Math.ceil(Math.random()*69);
        console.log("repeat number")
    }
    lotteryTicket.push(num);
}
lotteryTicket.sort(function(a, b){return a-b}); //(a, b) => {a - b}; 
// console.log(lotteryTicket);
let numSpec = Math.ceil(Math.random()*26);
lotteryTicket.push(numSpec);
// console.log(lotteryTicket);
for (let i = 0; i<lotteryTicket.length; i++) {
    if (lotteryTicket[i] < 10){
        lotteryTicket[i]=lotteryTicket[i].toString().padStart(2,"0");
    }
}
console.log(lotteryTicket);
...
// let bj6 = [];
// for (i=0; i<6; i++){
//     bj6 = bj6.concat(blackjack);
// }

const shoe = [...blackjack, ...blackjack, ...blackjack, ...blackjack, ...blackjack, ...blackjack,]
shoe.sort(function(a,b){return Math.random() - 0.5});
console.log(shoe);


// bj6.sort(function(a,b){return Math.random() - 0.5})
// console.log(bj6);
...
for (cit of cities) {
    for (cit2 of cities) {
        cityPair.push(`${cit}-${cit2}`)
    }
}

console.log("flights includes self pairs (NY-NY)", cityPair);
...
for (let cit of cities) {
    for (let cit2 of cities) {
        if (cit != cit2){
        cityPair2.push(`${cit}-${cit2}`)
        }

    }
}
console.log("Does NOT include self pairs (NY-NY)", cityPair2)
...
for (let ingr of froots){
    for (let base of froots){
        if (ingr != base){
            let temp = `${base}-${ingr}`
            if(smoothies.includes(temp)){
                continue
            } else {
                smoothies.push(`${ingr}-${base}`)
            }
        } 
    }
} 
...
console.log('fruitSmoothies:', smoothies);
