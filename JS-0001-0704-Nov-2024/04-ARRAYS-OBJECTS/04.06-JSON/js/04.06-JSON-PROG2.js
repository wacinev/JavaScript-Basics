...
// make animalsObj as one object -- no array
// keep all actual data values (can add/remove keys)

// animalsArr of 6 objects refactored as
// animalsObj of 6 properties (key-value pairs)
const animalsObj = {
  'American bison': {
    class: 'mammal', 
    herbivore: true, 
...
console.log(animalsObj.giraffe.class); // mammal 
// (via 'giraffe')
console.log(animalsObj['American bison'].class); // mammal // (via 'American bison')
console.log(animalsObj['Green Anaconda'].class); // reptile
console.log(animalsObj['American bison'].continent); // North America
console.log(animalsObj.giraffe.wildPop); // 70000
console.log(animalsObj.giraffe); // whole giraffe object
...
// CHALLENGE: get random animal object -- the whole object
// HINT: start by generating integer in range of animalKeysArr (0-3)
let r = ~~(Math.random() * animalKeysArr.length);
let randAnimal = animalKeysArr[r]; // giraffe or penguin etc
//  rand animal name as dynamic key to look up that animal:
const randAnimalObj = animalsObj[randAnimal];
console.log('randAnimalObj:', randAnimalObj);

// get the keys of indivual animal obj: class, herbivore, continent, etc.
const animalObjKeysArr = Object.keys(randAnimalObj);
console.log('animalObjKeysArr:', animalObjKeysArr);
...
console.log('typeof(animalsArr):', typeof(animalsArr)); 
console.log('animalsArr:\n', animalsArr);

const animalsJSON = JSON.stringify(animalsArr);
console.log('typeof(animalsJSON):', typeof(animalsJSON)); 
...
const animalsObjArrFromJson = JSON.parse(animalsJson);

// 5. Log the result and check the Console. We are back where we started:
console.log('animalsObjArrFromJson:\n', animalsObjArrFromJson[2]);
console.log(animalsObjArrFromJson[3].class); // bird (penguin)
console.log(animalsObjArrFromJson[2].continent); // Africa (giraffe)
...
const shallowAnaconda = animalsArr[1];
console.log('shallowAnaconda:\n', shallowAnaconda);

// 7. Make a few changes to the Shallow anaconda object:
// "ambushing prey in shallow water";
shallowAnaconda.wildPop++; // increment by 1
shallowAnaconda.behaviors = ['ambusing prey in shallow water', 'crushing hapless victims', 'slithering', 'basking in sun'];

// 8. Log anaconda as well as animals:
console.log('shallowAnaconda:\n', shallowAnaconda);
// check if original anaconda object also changed: it did!
console.log('animalsArr:\n', animalsArr);

/* Not only did shallow anaconda pick up the changes, but so did the original anaconda object in the animals array, which was copied to make the "stand-alone" anaconda.
...
*/
const deepAnaconda = JSON.parse(JSON.stringify(animalsArr[1]));
console.log('deepAnaconda:\n', deepAnaconda);
deepAnaconda.wildPop++; // increment by 1
deepAnaconda.behaviors.push('consume small prey smoothies');
console.log('deepAnaconda:\n', deepAnaconda);
// check if original anaconda object also changed: it did NOT!
console.log('animalsArr:\n', animalsArr);

// another way to make a deep copy is w ... destructuring
// which you wrap in restructing {} []
const deepPengin = { ...animalsArr[3] };
deepPengin.wildPop++; // increment by 1 
deepPengin.behaviors = ['waddle around', 'sit on eggs', 'eat fish'];
console.log('deepPengin:\n', deepPengin);
// check if original penguin object also changed: it did NOT!
console.log('animalsArr:\n', animalsArr);
